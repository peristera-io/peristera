...
locals {
  cloud_init = <<-CLOUD
    ...
    package_update: true
    runcmd:
      - curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="--flannel-backend=none --disable-network-policy --tls-san ${scaleway_instance_ip.node.address} --write-kubeconfig-mode 644" sh -
  CLOUD
}
